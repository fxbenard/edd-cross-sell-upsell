<?php
/**
 * Cross-sell/Upsell Log View Class
 *
 * @package     EDD
...
/**
 * EDD_CSAU_Log_Table Class
 *
 * Renders the cross-sell/upsell log list table
 *
 * @since 1.1
 */
class EDD_CSAU_Log_Table extends WP_List_Table {
	...
	public function get_columns() {
		$columns = array(
			'ID'		=> __( 'Log ID', 'edd-csau' ),
			'user_id'  	=> __( 'User', 'edd-csau' ),
			'download'  => __( 'Download', 'edd-csau' ),
			'amount'    => __( 'Item Amount', 'edd-csau' ),
			'payment_id'=> __( 'Payment ID', 'edd-csau' ),
			'date'  	=> __( 'Date', 'edd-csau' )
		);
	...
	public function downloads_filter() {
		...
		if ( $downloads ) {
			echo '<select name="download" id="edd-log-download-filter">';
				echo '<option value="0">' . __( 'All', 'edd-csau' ) . '</option>';
	...
	public function prepare_items() {
		...
		$total_items           = $edd_logs->get_log_count( $this->get_filtered_download(), $this->type, $this->get_meta_query() );
